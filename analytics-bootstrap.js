(function initStopTheSlopAnalytics() {
  const connectionString = String(window.STS_APP_INSIGHTS_CONNECTION_STRING || "").trim();
  ...
  if (!connectionString) {
    window.STSAnalytics = buildNoopAnalytics();
    return;
  }
  ...
  !(function (cfg) {
    ...
    "instrumentationKey"[x = "toLowerCase"](), w = "crossOrigin", D = "POST", t = "appInsightsSDK", E = cfg.name || "appInsights", (cfg.name || C[t]) && (C[t] = E), n = C[E] || function (g) {
      var f = false, m = false, h = { initialize: true, queue: [], sv: "8", version: 2, config: g };
      function v(e, t) {
        var n = {}, i = "Browser";
        function a(e) {
          e = "" + e;
        ...
        return n[j + "id"] = i[x](), n[j + "type"] = i, n["ai.operation.name"] = b && b.pathname || "_unknown_", n["ai.internal.sdkVersion"] = "javascript:snippet_" + (h.sv || h.version), { time: (i = new Date).getUTCFullYear() + "-" + a(1 + i.getUTCMonth()) + "-" + a(i.getUTCDate()) + "T" + a(i.getUTCHours()) + ":" + a(i.getUTCMinutes()) + ":" + a(i.getUTCSeconds()) + "." + (i.getUTCMilliseconds() / 1e3).toFixed(3).slice(2, 5) + "Z", iKey: e, name: "Microsoft.ApplicationInsights." + e.replace(/-/g, "") + "." + t, sampleRate: 100, tags: n, data: { baseData: { ver: 2 } }, ver: undefined, seq: "1", aiDataContract: undefined };
      }
      ...
      }, l && !cfg.useXhr ? l(d, { method: "GET", mode: "cors" }).then(function (e) { return e.json()["catch"](function () { return {}; }); }).then(t)["catch"](r) : XMLHttpRequest && ((a = new XMLHttpRequest).open("GET", d), a.onreadystatechange = function () { if (a.readyState === XMLHttpRequest.DONE) if (200 === a.status) try { t(JSON.parse(a.responseText)); } catch (e) { r(); } else r(); }, a.send())) : o && r();
      try {
        h.cookie = O.cookie;
      } catch (k) { /* ignore */ }
      function e(e) {
      ...
      return !0 !== g[L] && !0 !== l[L] && (e(["_" + (c = "onerror")]), u = C[c], C[c] = function (e, t, n, i, a) {
        var o = u && u(e, t, n, i, a);
        return !0 !== o && h["_" + c]({ message: e, url: t, lineNumber: n, columnNumber: i, error: a, evt: C.event }), o;
  ...
  window.STSAnalytics = {
    ...
    trackEvent(name, properties, measurements) {
      ...
      if (!ai || typeof ai.trackEvent !== "function" || !name) return;
      ai.trackEvent({
        name,
    ...
    trackPageView(name, uri, properties, measurements) {
      const ai = window.appInsights;
      if (!ai || typeof ai.trackPageView !== "function") return;
      ai.trackPageView({
        name: String(name || document.title || "").trim() || undefined,
        uri: String(uri || window.location.href || "").trim() || undefined,
        properties: sanitizeProperties(properties),
        measurements: sanitizeMeasurements(measurements),
      });
    },
    setUserContext(user) {
      ...
      if (!ai || typeof ai.setAuthenticatedUserContext !== "function" || !userId) return;
      ai.setAuthenticatedUserContext(userId);
    },
    clearUserContext() {
      const ai = window.appInsights;
      if (!ai || typeof ai.clearAuthenticatedUserContext !== "function") return;
